# https://github.com/basomingera/MM1-queue-simulator/blob/master/MM1.py
import random
import csv
import copy
...
TOTAL_SIMULATION_TIME = 100000  # Tiempo de ejecución de la simulación
DEBUG = False  # True para registros detallados


# Class para crear un objeto que representa un solo trabajo
class Job:
    def __init__(self, arrival_time, job_id, job_size):
        ...
        self.queue_time = 0
        self.status = 0  # 0 creado, 1 cola, 2 en proceso, 3 completado
...
class System:
    def __init__(self, service_rate):
        ...
        self.latest_job_service_ending_time = 0  # inicialmente no job
        self.queue_list = []
    ...
    def handle_jobs(self, the_new_job):
        ...
        for this_job in temp_copy_of_jobs_in_sys:
            if this_job.service_start_time <= current_time and this_job.status < 2:
                if DEBUG:
                    print("Tiempo: " + str(this_job.service_start_time) + "segundos \t\tJod Id: " + str(
                        this_job.job_id) + " Empieza a procesar ..... ")
                self.queue_summary_over_time[current_time] = len(self.queue_list)
                this_job.status = 2
                if this_job.service_end_time <= current_time:
                    this_job.status = 3
                    if DEBUG:
                        print("Tiempo: " + str(this_job.service_end_time) + "segundos \t\tJod Id: " + str(
                            this_job.job_id) + " Proceso terminado, el tamaño de la cola es: " + str(len(self.queue_list) - 1))
        ...
        if not new_job_inserted:
            # añadir el trabajo actual a los trabajos del sistema
            self.queue_list.append(the_new_job)
            # se actualiza el resumen de la cola
            self.queue_summary_over_time[current_time] = len(self.queue_list)

            if DEBUG:
                print("Tiempo: " + str(current_time) + "segundos \t\tJod Id: " + str(
                    the_new_job.job_id) + " Entrada al sistema, system job size is: " + str(len(self.queue_list)))
